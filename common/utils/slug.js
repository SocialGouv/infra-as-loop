...
const slugString = (name, len, defaultPrefix) => {
  let slugified = slugify(name, {
    lower: true,
  })

  if (!/^[a-z]/.exec(slugified)) {
    slugified = `${defaultPrefix}-${slugified}`
  }
...
const slug = (mixed, options = {}) => {
  ...
  for (const part of mixed) {
    ...
    const slugified = slugString(name, pml, defaultPrefix)
    parts.push(slugified)
  }

  const slugified = slugString(parts.join(glue), maxLength, defaultPrefix)
